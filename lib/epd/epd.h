...
class __RoundRect {
	private:
	unsigned char innerColor = WHITE, outlineColor = BLACK;
	int radius = 10;
	public:
	void setColor(unsigned char innerColor, unsigned char outlineColor);
	void setRadius(int r);
	void draw(int x0, int y0, int x1, int y1);
};

/**
 * use RoundRect.draw(x0, y0, x1, y1) to draw a round rect
*/
extern __RoundRect RoundRect;
